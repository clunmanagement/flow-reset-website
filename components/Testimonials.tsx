      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {tx.testimonials.items.map((t, i) => (
            <motion.div key={t.name} initial={{ opacity: 0, y: 30 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: i * 0.15 }} className="bg-white/10 rounded-[2rem] p-8 flex flex-col gap-6 backdrop-blur-sm hover:bg-white/15 transition-colors">
